...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++){
        ...
        let gameDescription = games[i].description;
        let gameImg = games[i].img;
        let gameBackers = games[i].backers;
        gameCard.innerHTML = `<img class="game-img" src="${gameImg}" alt="image of ${gameName}">
        <p>${gameName}</p>
        <p>${gameDescription}</p>
        <p>Backers: ${gameBackers}`
...
function filterUnfundedOnly() {
    ...
    let noGoal = GAMES_JSON.filter((game) => {
...
let displayString = 
    `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames} ${totalGames == 1 ? "game" : "games"}. Currently ${numUnfunded} ${numUnfunded == 1 ? "game" : "games"}
${numUnfunded == 1 ? "remains" : "remain"} unfunded. We need your help to fund these amazing games!`
...
let top2 = [firstGame, secondGame];

// create a new element to hold the name of the top pledge game, then append it to the correct element
let topGame = top2[0].name;
